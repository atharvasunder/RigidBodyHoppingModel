...
class RbsAnimation:
    ...
    def update_animation(self, t, rigid_body_system):
        ...
        if t >= self.next_time:  # not sure why this is called
        ...
        # force figure update
            self.fig.canvas.restore_region(self.bg) # restore the initialized image

            for ix in range(len(self.body_lines)):
                self.ax.draw_artist(self.body_lines[ix])
